...
const demoSites = [
  {
    ...
    body: "A warm animated cafe landing page with menu cards, product visuals, intro video, and smooth reveal motion.",
  },
  {
...
export default function AxiomExperience() {
  ...
  return (
    <main ref={root} className="bg-[#050505] text-[#f4f1ea]">
      ...
      <section id="demos" className="demo-sites-section px-5 py-24 md:px-10 md:py-32">
        <div className="mx-auto max-w-6xl">
          <div className="mx-auto max-w-4xl text-center">
            <p className="mb-8 text-[10px] uppercase tracking-[0.38em] text-white/36">Demo websites</p>
            <h2 className="font-serifDisplay text-[clamp(3rem,6.8vw,7rem)] leading-[0.92] tracking-[-0.035em]">
              Visit every live demo from Velora.
            </h2>
            <p className="mx-auto mt-7 max-w-2xl text-lg leading-8 text-white/52 md:text-xl">
              This is the main Velora site. Use the links below to open the other websites and deployments.
            </p>
          </div>
            ...
            {demoSites.map((site, index) => (
              <a className="demo-site-card group" href={site.href} target="_blank" rel="noreferrer" key={site.title}>
                <div className="flex items-start justify-between gap-5">
                  <span className="text-[10px] font-bold uppercase tracking-[0.24em] text-white/40">{site.label}</span>
                  <span className="font-serifDisplay text-5xl leading-none text-white/18">{String(index + 1).padStart(2, "0")}</span>
                </div>
                <h3 className="mt-10 font-display text-4xl font-black uppercase leading-[0.9] tracking-[-0.05em] text-white md:text-5xl">
                  {site.title}
                </h3>
                <p className="mt-6 min-h-28 text-base leading-7 text-white/52">{site.body}</p>
                <div className="mt-9 inline-flex items-center gap-3 border-b border-white/28 pb-2 text-[10px] font-bold uppercase tracking-[0.24em] text-white transition group-hover:border-white">
